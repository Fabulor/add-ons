...
## updated for Fabulor by Barry Suridge (barry.suridge@gmail.com)
##
## What is this?
...
# Fabulor-Name: Nicenicks
# Fabulor-Version: 0.11
# Fabulor-Description: Colourize nicks based on least-recently-used colour

"""Fabulor add-on for colourizing nicks based on least-recently-used colour."""
...
def save_colors():
    if not hexchat.set_pluginpref(
        PREF_KEY_COLORS, json.dumps(permacolortable, sort_keys=True)
    ):
...
def get_color(ctable, nick):
    ...
    for i in range(len(ctable) - 1, -1, -1):
        c, n = ctable[i]
        if pcolor is not None and c == pcolor:  # if we found this nick's permcolor
            ...
            if pcolor is not None:  # if this nick has a color in the table different from its permacolor
                ...
                dmsg(
                    nick
                    + "'s permacolor was found to be different from the one in the table, and was reassigned from "
                    + str(c)
                    + " to "
                    + str(pcolor),
                    "GETCOLOR",
                ...
                dmsg(
                    nick + "'s color was found in the colortable for this channel.",
                    "GETCOLOR",
                )
    ...
    if color is None:
        ...
        color = c
        dmsg(
            "A new entry was added to this colortable: " + nick + " -> " + str(c),
            "GETCOLOR",
        )
...
def setcolor_command(word, word_eol, userdata):
    ...
    if paramcount < 1:  # no parameters, so display all the current nick colours
        ...
        if len(items) > 0:
            ...
            omsg(
                "No nick colour mappings assigned. Type /HELP SETCOLOR for more info.",
                "PERMA-COLORS",
            )
        return hexchat.EAT_ALL
    ...
    if paramcount == 1:  # just the nick was supplied
        ...
        if 0 <= color <= MAX_COLOR:
            ...
            omsg(
                "Not a valid colour! Please pick one between 0 and 99. See the 'Preferences...' for the list of colours.",
                "ERROR",
            )
    ...
    for network, channels in chancolortable.items():
        ...
        for channel, nicks in channels.items():
            ...
            for color, nick in nicks:
                if nick is not None:
                    this_channel_table.append(
                        "".join([col(color), nick, ecs("o"), ": ", str(color)])
                    )
...
def tab_focus_callback(word, word_eol, userdata):
    ...
    dmsg(
        "Focus changed to {}. Current color3_tabs: {!r}".format(
            current_focus_tab, color3_tabs
        ),
        "GUICOLOR",
    )
    ...
    if nremoved:
        dmsg(
            "Removed {} tab{} from color3_tabs.".format(
                nremoved, "s" * (nremoved != 1)
            ),
            "GUICOLOR",
        )
    return hexchat.EAT_NONE
...
def message_callback(word, word_eol, userdata, attributes):
    ...
    if nicenicks_enabled:
        ...
        dmsg(
            "The time attribute for this event is {}".format(attributes.time),
            "PRINTEVENT",
        )
        ...
        if chan not in chancolortable[net]:
            ...
            dmsg(
                ...
                + str(len(chancolortable[net][chan]))
                + " for channel "
                + chan
                + " on network "
                + net,
        ...
        dmsg(
            "COLORTABLE for " + chan + " on " + net + " = " + str(ctable), "COLORTABLE"
        )
        ...
        if ctable:
            for i, (color, nick) in enumerate(ctable):
                if nick and nick.lower() == oldnick.lower():
                    ...
                    dmsg(
                        "Nick change, table updated: {0}{1}{3} -> {0}{2}{3}".format(
                            col(color), oldnick, newnick, ec["o"]
                        ),
                        "NICKCHANGE",
                    )
                    return hexchat.EAT_NONE
...
hexchat.hook_print_attrs(
    "Channel Message", message_callback, "Channel Message", priority=hexchat.PRI_HIGHEST
)
hexchat.hook_print_attrs(
    "Channel Action", message_callback, "Channel Action", priority=hexchat.PRI_HIGHEST
)
hexchat.hook_print_attrs(
    "Channel Msg Hilight", tab_hilight_callback, priority=hexchat.PRI_LOW
)
hexchat.hook_print_attrs(
    "Channel Action Hilight", tab_hilight_callback, priority=hexchat.PRI_LOW
)
...
hexchat.hook_command(
    ...
    None,
    hexchat.PRI_NORM,
    "Usage:\t/NICEDEBUG On to enable for all messages, /NICEDEBUG Off to disable, or to enable showing only debug messages with a certain description: '/NICEDEBUG description'. Remove a description: '/NICEDEBUG -description'",
)
hexchat.hook_command(
    "SETCOLOR",
    setcolor_command,
...
hexchat.hook_command(
    "NICENICKS_DUMP",
    nicenicks_dump_command,
    None,
    hexchat.PRI_NORM,
    "Usage:\t/NICENICKS_DUMP to dump all the nick colours for all active channels. To show the raw table without colours (might not fit on one line): /NICENICKS_DUMP raw",
)
...
if not nicenicks_enabled:
    print(
        "+\tTo have Nicenicks enabled on start, do '/set text_color_nicks 1', or turn that setting on at:"
    )
